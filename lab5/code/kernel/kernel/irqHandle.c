...
void syscallOpen(struct StackFrame *sf) {
	...
	if (ret == 0) { // file exist
		...
		if ((((sf->edx >> 3) & 1) == 0 && destInode.type == DIRECTORY_TYPE) ||   
			(((sf->edx >> 3) & 1) == 1 && destInode.type == REGULAR_TYPE)) { 
...
void syscallLseek(struct StackFrame *sf) {
	...
	switch(sf->ebx) { // whence
		...
		case SEEK_CUR:
			// TODO: if SEEK_CUR
			if(file[idx].offset + offset >= 0 && file[idx].offset + offset <= inode.size){
				file[idx].offset += offset;
				pcb[current].regs.eax = 0;
			}
			else
